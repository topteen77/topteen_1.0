...
function handleCareerResults(result, index) {
  ...
  const slicedEntries = entries.slice(0, -2); // Excludes the last two entries

  for (const [dimension, data] of slicedEntries) {
    const score = data.score || 0;

    const cleanCode = dimension.replace(/\d+$/, ''); // Removes trailing numbers

    const name = riasecNames[cleanCode] || data.name || dimension;
    dimensions.push({
      code: cleanCode,
      name: name,
      score: score,
  ...
  topThree.forEach((dim, index) => {
    ...
    card.style.borderRadius = '8px'; // Optional: add some border radius for better look
...
function handleAptitudeResults(result, index) {
  ...
  // Get the main response that contains section data
  ...
  if (mainResponse && mainResponse.selected_answer.sections) {
    const sectionsDataRaw = mainResponse.selected_answer.sections;

    // Process each section
    Object.entries(sectionsDataRaw).forEach(([sectionName, sectionData]) => {
      const sectionScore = sectionScores[sectionName] || 0;
      const answers = sectionData.submitted_answers || {};
      const totalQuestions = Object.keys(answers).length;
      const correctAnswers = Object.values(answers).filter(ans =>
    ...
    // Fallback: Process sections based on section scores only
    Object.entries(sectionScores).forEach(([sectionName, sectionScore]) => {
      sectionsData.push({
        name: sectionName,
        score: sectionScore,
        totalQuestions: 15, // Default assumption
        correctAnswers: sectionScore,
        accuracy: (sectionScore / 15) * 100
      });
    });
  }
...
document.addEventListener('DOMContentLoaded', function () {
  // Try to load from context first, then API
