...
from institute.decorators import institute_user_only,institute_authenticated_user_only,institute_block_student_only,institute_update_delete_student_only,institute_change_student_password_only,institute_profile_update_delete,only_superuser,institute_group_user_only
...
def dashboard(request):
    try:
        ...
        if above_avg_score in above_avg:
            ...
            elif 'EMOTIONAL' in above_avg_score:
...
def test3_view(request):
    ...
    if request.method == 'POST':
        ...
        if test_paper == 'test3':
            ...
            logical_questions = Question.objects.filter(category='Logical')
            verbal_questions = Question.objects.filter(category ='Verbal')
            numerical_questions = Question.objects.filter(category='Numerical')
            emotional_questions = Question.objects.filter(category='Emotional')
            language_questions = Question.objects.filter(category='Language')
            spatial_questions = Question.objects.filter(category='Spatial')
            mechanical_questions = Question.objects.filter(category='Mechanical')
            ...
            try:
                ...
                # Handle the case where the result doesn't exist (optional)
                test3_result = Results(user=request.user, test_paper='test3', scores={}, results={})
                personality = {}

            logical_score = personality.get('logical_score', 0)
            verbal_score = personality.get('verbal_score', 0)
            numerical_score = personality.get('numerical_score', 0)
            emotional_score = personality.get('emotional_score', 0)
            language_score = personality.get('language_score', 0)
            spatial_score = personality.get('spatial_score', 0)
            mechanical_score = personality.get('mechanical_score', 0)

            for category_questions in [logical_questions, verbal_questions, numerical_questions, emotional_questions, language_questions, spatial_questions, mechanical_questions]:
        ...
        # if test_completion.spatial_complete == True:
        #     test_completion.test3_complete = True
        #     test_completion.save()
        # else:
        #     test_completion.spatial_complete = False
...
def generate_pdf(request):
    ...
    if request.method == 'POST':
        test_paper = request.POST.get('test_paper')
        questions = Question.objects.filter(test_paper=test_paper)
        print('test_paper',test_paper,"questions",questions)
        ...
        if test_paper == 'test1':
            ...
            print(f"sum_R: {sum_R}, sum_I: {sum_I}, sum_A: {sum_A}, sum_S: {sum_S}, sum_E: {sum_E}, sum_C: {sum_C}")
            ...
            # Print the results
            print(f"test1_result.Results: {results}")
            print("######################################################################")
            ...
            print('test1_result', test1_result)           
...
def submit_clicks(request):
    if request.method == 'POST':
        try:
            ...
            test_completion.save()
            print("test_completion_2", test_completion.test2_complete)
            print('test3_result',test2_result)           
...
def app_submit(request):
    ...
    if request.method == 'POST':
        ...
        if test_paper == 'test1':
            ...
            test_completion.save()
            print("test_completion_1", test_completion.test1_complete)
            ...
            test_completion.test3_complete = True
            test_completion.save()
            print("test_completion_3", test_completion.test3_complete)
...
def download_pdf(request,test_paper):
    ...
    if questions is not None:
        ...
        if test_paper == 'test1':
            response['Content-Disposition'] = f'attachment; filename="{user_name}-Personality_Assessment_report.pdf"'
        elif test_paper == 'test2':
            response['Content-Disposition'] = f'attachment; filename="{user_name}-Interest_Assessment_report.pdf"'
        elif test_paper == 'test3':
            response['Content-Disposition'] = f'attachment; filename="{user_name}-Intelligence_Assessment_report.pdf"'
        else:
            response['Content-Disposition'] = f'attachment; filename="{user_name}-Final_Assessment_report.pdf"'
