...
# Where to write "Understanding" static includes for all four (project dir = topteen_1.0)
_PROJECT_ROOT = _SCRIPT_DIR.parent.parent
UNDERSTANDING_INCLUDES_DIR = _PROJECT_ROOT / "templates" / "template20" / "includes"
ENGAGEMENT_UNDERSTANDING_INCLUDE = UNDERSTANDING_INCLUDES_DIR / "engagement_patterns_understanding_guide.html"
...
def generate_all_understanding_data():
    ...
    for _slug, html_path, title, json_filename in UNDERSTANDING_DATA_CONFIG:
        ...
        # Write full Understanding fragment (all accordions: Primary + Combination + Maximizing) for each assessment
        UNDERSTANDING_INCLUDES_DIR.mkdir(parents=True, exist_ok=True)
        include_path = UNDERSTANDING_INCLUDES_DIR / f"{_slug}_understanding_guide.html"
        include_path.write_text(fragment, encoding="utf-8")
        print("Wrote", include_path)
